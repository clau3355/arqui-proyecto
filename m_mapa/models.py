...
os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = 'fastdeliveryproject-0ff8eca1ffb4.json'

client = bigquery.Client()

class User:
    def __init__(self,username,password,mail,id_user,ubicacion,money):
...
def ObtenerUsuarios():
    ...
    for row in query_job.result():
        users.append(User(username=row["username"], ubicacion=row["ubicacion"], money=row["money"], password=row["pass"],id_user=row["id_user"], mail=row["correo"]))
    return users
...
def AñadirUser(a,b,c):
   # For this sample, the table must already exist and have a defined 
    ...
    rows_to_insert = [("4",b,"100",None,fecha,a,c)]
